          ...
          <div class="content" v-for="(data,index) in renderLists" :key="data.id">
            ...
            <div v-if="data.fqmcategory === 1" class="checkWrap">
                ...
                <!-- <p>{{item.vanswer}}</p> -->
                    <div v-if="item.iinputtype === 0">
                       <input v-model='data.checkbox' :value='item.vquestcontent' :id='"demo"+index+oindex' type="checkbox" :name="'checkbox'+index" @click="changes(oindex,data,index)" disabled="disabled">
                       <label :for='"demo"+ index + oindex' v-text='item.vquestcontent' :class="item.checkStatus"></label>
                    </div>
                    <div v-if="item.iinputtype === 1">
                       <input type="text" :name="'input'+index" placeholder="其他" class="checkbox-text" v-model="item.vanswer" maxlength="50" disabled="disabled">
                    </div>
                </div>
            </div>
            <div v-else class="radioWrap">
               <div v-for='(item,oindex) in data.option'>
                   <!-- <p>{{item.vanswer}}</p> -->
                   <div v-if="item.iinputtype === 0">
                        <input class="self-radio" type="radio" :id="'radio'+ index +oindex" :name="'radio' + index" v-model="data.val" :value='item.vquestcontent' @click="change(oindex,data,index)" disabled="disabled">
                        <label :for='"radio"+ index + oindex'>{{item.vquestcontent}}</label>
                   </div>
                   <div v-if="item.iinputtype === 1">
                        <input type="text" name="" placeholder="其他" class="checkbox-text" v-model="item.vanswer"  maxlength="50" disabled="disabled">
                    </div>
                </div>
            </div>             
          </div>      
...
<script>
  ...
  import {getQuestionContent} from '@/api/getData'
...
<style scoped>
   ...
   .gzb-list-title {
    ...
    .radioWrap input[type='radio']:checked+label {
      background: url('../../assets/radioStatus.png') no-repeat left;
      background-size: 18px 18px;
      padding-left: 24px;
      display: inline-block;
      margin-left: -5px;
   ...
   /* .checkWrap input[type='checkbox']:checked+label {
    ...
    .checkBoxs {
      background: url('../../assets/checkStatus.png') no-repeat left !important;
