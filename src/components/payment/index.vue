<template>
  <div class="hello" :class="hasContent">
    <div class="gzb-nav">
        <div class="gzb-nav-left kitbox">
            <div class="gzb-nav-item kitbox" @click="showList">
                <span class="gzb-arrow">全部状态</span>
            </div>
            <div class="gzb-nav-item kitbox">
                <router-link to='/select/payment'><span class="gzb-arrow">{{dateLabel}}</span></router-link>
            </div>
            <clear></clear>
        </div>
    ...
    <div class="gzb-travel-list"  v-for="group in renderList" :key="group.id" slot="list">
        ...
        <paymentItem v-for="app in group.data" :key="app.id" :title="app.description === ''? '无' : app.description" :number="app.total" :status="app.billStatus" :color="app.color" :id="app.id" ></paymentItem>
    </div>
...
import {XuntongJSBridge} from '@/js/qingjs'
  export default {
    ...
    methods: {
      chooseList(index) {
          let searchInfo = this.$store.state.payment
          searchInfo.billStatus = this.statusList[index].code
          searchInfo.statusIndex = index
          this.$store.commit('setPaymebt', searchInfo)
          this.appList = []
          this.renderList = []
          this.page_no = 1
      ...
      toAdd() {
          this.$store.commit('emptyPayment')
      ...
      async getAPPList() {
          ...
          let params = {
              user_id: this.$store.state.userInfo.user_id,
              begin_date: this.$store.state.payment.beginDate,
              end_date: this.$store.state.payment.endDate,
              bill_type: '264X-wangbao3',
              search_type: '',
              method: 'getList',
              page_no: this.page_no,
              page_size: 8,
              bill_status: this.$store.state.payment.billStatus
          }
    ...
    created: function() {
        XuntongJSBridge.call('setWebViewTitle', {'title': '资金支付'})
      this.loadList()
  },
    computed: {
        showstatus() {
            return this.statusList[this.$store.state.payment.statusIndex].name
        },
        dateLabel() {
            return this.dateList[this.$store.state.payment.dateIndex]
        }
    },
    components: {
      paymentItem,
